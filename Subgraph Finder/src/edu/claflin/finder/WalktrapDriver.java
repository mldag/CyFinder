...
//				"S.txt", "M.txt", "B.txt", "D.txt", "BRCA.txt", "COAD.txt", "GBM.txt"
		...
		for (String fn : fnames)
		{
			...
			Walktrap wt = new Walktrap(ab);
			wt.setMinNodeCount(g.getNodeCount() <= 10 ? 1 : 10);
			wt.setGraphSortOrder(GraphSortOrder.DESCENDING);

			double start = 0.0;
			double end = 0.0;

			start = System.currentTimeMillis();
			List<Graph> results = wt.process(g);
			end = System.currentTimeMillis();
			double wttime = (end - start) / 1000;
